...
A blueprint is a JSON document with five optional fields: `name`, `theme`, `roles`, `categories`, and `channels` (top-level ones, outside any category). Roles carry a name, a permission preset plus any extra permission names, and color, hoist, and mentionable, and are listed highest first: that order is the hierarchy. There is no position field because one is not needed. Discord parks every role created through the API at position 1 and ranks roles that share a position by id, older higher, so a build that creates the top role first reproduces the hierarchy exactly, and an export lists roles the same way. Channels carry a name, type (text, voice, forum, stage, announcement), topic, slowmode, nsfw, and the visibility sugar below. Blueprints are portable across guilds because they reference roles by name rather than by ID.

Limits match Discord's own: 250 roles, 500 channels, 50 categories, and 50 channels per category. The schema enforces them on authoring and export, so any server Discord allows fits in a blueprint, and the planner checks the same numbers against the live server, existing entities plus what the blueprint adds, since a blueprint that fits on its own can still overflow a server that is already half full.

What a blueprint does not carry: numeric role positions (order stands in for them, as above), AutoMod rules, onboarding, the welcome screen, scheduled events, and seed content such as a rules post or pinned message. Those are managed by their own tools, not by the builder.
...
| execute_build_plan | no (additive) | aggregate of the plan's needs, typically Manage Channels + Manage Roles | guild, plan_id or blueprint | Executes a plan. The blueprint is re-validated against live server state at execution time, so staged plans can never act on stale data. Strictly additive in v1: existing entities are reused, nothing is deleted or modified; visibility sugar compiles to permission overwrites at creation, and the bot always grants itself access to what it builds. Runs in dependency order (roles highest first, then categories, then channels), halts on failure with a created/failed/not-attempted report, and re-running after a fix resumes naturally through reuse. New roles are then given distinct positions in one bulk call when the bot's own role sits high enough for Discord to allow it; when it does not, which is the case on a fresh server, they share a position and the client still ranks them in blueprint order. `role_positions` in the result says which happened. Reconcile mode (destructive, drift-correcting) is deferred to the diff_blueprint work. |
